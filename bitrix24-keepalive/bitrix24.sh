#!/bin/bash
# MODIFIED FROM HERE: https://github.com/farOverNinethousand/networkDriveChecker
...
# SETTINGS
#===================================================================
#users='abc@def.com'   # Bitrix24 account - user/email with administrator rights - separated by semicolon (s1@ex.com;s2@ex.com;.....)
#passwords='abcdef' # Bitrix24 account - password - separated by semicolon (1234567;987654;.....)
#accounts='b24-xxxxx' # default names or custom bitrix24 - separated by semicolon (b24-xxxxxx;b24-xxxxxx;.....)
users="$DOCKER_USER"
passwords="$DOCKER_PASS"
...
function execCurl() {
  ...
  local statusCode=$(curl -k --write-out '%{http_code}' --anyauth --user ${3} --silent --output /dev/null -X "${1}" "${2}")
